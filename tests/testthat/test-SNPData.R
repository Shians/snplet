...
test_that("SNPData coverage calculations work correctly", {
    ...
    # Test m_allele_frac method
    expected_m_allele_frac <- Matrix::Matrix(matrix(c(0.5 + abs(5/6 - 0.5), 0.5 + abs(6/8 - 0.5),
                                                    0.5 + abs(7/10 - 0.5), 0.5 + abs(8/12 - 0.5)),
                                           nrow = 2, ncol = 2))
    expect_equal_unnamed(m_allele_frac(snp_data), expected_m_allele_frac)
})

test_that("SNPData count data frame methods work correctly", {
    snp_data <- SNPData(test_alt_count, test_ref_count, test_snp_info, test_sample_info)

    # Test cell_count_df method
    cell_df <- cell_count_df(snp_data)
    ...
    expect_true(all(c("snp_id", "donor_id", "ref_count", "alt_count", "total_count", "ref_ratio", "maf") %in% colnames(donor_df)))
    ...
    expect_true(all(c("snp_id", "clonotype", "ref_count", "alt_count", "total_count", "ref_ratio", "maf", "donor_id") %in% colnames(clonotype_df)))
})
...
test_that("SNPData handles missing IDs correctly", {
    ...
    snp_data <- SNPData(test_alt_count, test_ref_count, test_snp_info_no_id, test_sample_info_no_id)
...
test_that("SNPData validates input dimensions", {
    ...
    expect_error(SNPData(wrong_dim_alt_count, test_ref_count, test_snp_info, test_sample_info),
                 "nrow\\(alt_count\\) == nrow\\(ref_count\\)")

    wrong_dim_snp_info <- data.frame(snp_id = c("snp_1", "snp_2", "snp_3"))
    expect_error(SNPData(test_alt_count, test_ref_count, wrong_dim_snp_info, test_sample_info),
