...
# Helper function to build SNPData with a single NA donor for edge cases
create_snpdata_with_na_donor <- function() {
    barcode_info <- test_barcode_info
    barcode_info$donor[1] <- NA
    SNPData(
...
expect_maf_columns_added <- function(result_df) {
    ...
    # Verify test_maf function columns are present
...
# Helper function to validate MAF columns are absent
expect_maf_columns_absent <- function(result_df) {
    expected_maf_cols <- c("minor_allele_count", "p_val", "adj_p_val")
    # Verify test_maf columns are not present
    expect_false(any(expected_maf_cols %in% colnames(result_df)))
}
...
test_that("barcode_count_df returns a tibble with expected columns", {
    # Setup
    snp_data <- SNPData(
        alt_count = test_alt_count,
        ref_count = test_ref_count,
...
test_that("donor_count_df aggregates by donor correctly", {
    # Setup
    snp_data <- SNPData(
        alt_count = test_alt_count,
        ref_count = test_ref_count,
...
test_that("SNPData constructor formats duplicate warning when many duplicates", {
    dup_alt_count <- Matrix::Matrix(matrix(1, nrow = 7, ncol = 1))
    dup_ref_count <- Matrix::Matrix(matrix(2, nrow = 7, ncol = 1))
    dup_snp_info <- data.frame(
        snp_id = rep("dup_snp", 7),
        pos = seq_len(7),
        stringsAsFactors = FALSE
    )
    ...
    # Verify warning includes duplicate count when more than 5 duplicates exist
    expect_warning(
        snp_dup <- SNPData(
            alt_count = dup_alt_count,
            ref_count = dup_ref_count,
    ...
    # Verify duplicates are removed down to a single SNP
    expect_equal(nrow(snp_dup), 1)
})
...
test_that("filter_snps errors when no SNPs remain after filtering", {
    # Setup
    snp_data <- SNPData(
        alt_count = test_alt_count,
        ref_count = test_ref_count,
    ...
    # Verify empty SNP selection errors
    expect_error(
        filter_snps(snp_data, pos > 1000),
        "No SNPs remain after filtering"
    )
})

test_that("filter_barcodes errors when no barcodes remain after filtering", {
    # Setup
    snp_data <- SNPData(
        alt_count = test_alt_count,
        ref_count = test_ref_count,
    ...
    # Verify empty barcode selection errors
    expect_error(
        filter_barcodes(snp_data, donor == "donor_missing"),
        "No barcodes remain after filtering"
    )
})

test_that("filter_samples forwards to filter_barcodes", {
    # Setup
    snp_data <- SNPData(
        alt_count = test_alt_count,
        ref_count = test_ref_count,
...
test_that("aggregate_count_df works correctly with various grouping columns", {
    ...
    ref_cell1_snp1 <- test_ref_count[1, 1]
    ref_cell2_snp1 <- test_ref_count[1, 2]
    alt_cell1_snp1 <- test_alt_count[1, 1]
    ...
    expect_maf_columns_absent(no_maf_df)
    ...
    expect_maf_columns_added(with_maf_df)
})
...
test_that("aggregate_count_df handles edge cases correctly", {
    ...
    # Verify warning about NA values is logged
...
test_that("test_maf=TRUE adds correct statistical columns to aggregate_count_df", {
    ...
    expect_maf_columns_added(result_clonotype_maf)
})
...
create_barcode_merge_test_data <- function() {
    alt_x <- Matrix::Matrix(
        matrix(
            c(
                1, 0, # snpA
            ...
            nrow = 2,
            ncol = 2,
            dimnames = list(c("snpA", "snpB"), c("cell_x1", "cell_x2"))
        )
    )
    ref_x <- Matrix::Matrix(
        matrix(
    ...
    alt_y <- Matrix::Matrix(
        matrix(
            ...
            nrow = 2,
            ncol = 2,
            dimnames = list(c("snpA", "snpB"), c("cell_y1", "cell_y2"))
        )
    )
    ref_y <- Matrix::Matrix(
        matrix(
...
test_that("merge_snpdata merges by barcode when barcode column exists", {
    data <- create_barcode_merge_test_data()
    x <- data$x
    y <- data$y
    ...
    # Verify cell_id is regenerated when joining by barcode
    expect_equal(merged_barcode_info$cell_id, c("cell_1", "cell_2", "cell_3"))

    # Verify counts for shared barcode bc2 are summed
    ref_x <- ref_count(x)
    ref_y <- ref_count(y)
    expected_ref <- Matrix::Matrix(
        matrix(
            c(
                ref_x[1, "cell_x1"],
                ref_x[1, "cell_x2"] + ref_y[1, "cell_y1"],
                ref_y[1, "cell_y2"],
                ref_x[2, "cell_x1"],
                ref_x[2, "cell_x2"] + ref_y[2, "cell_y1"],
                ref_y[2, "cell_y2"]
            ),
            nrow = 2,
            ncol = 3,
            byrow = TRUE,
            dimnames = list(c("snpA", "snpB"), c("cell_1", "cell_2", "cell_3"))
        )
    )
    alt_x <- alt_count(x)
    alt_y <- alt_count(y)
    expected_alt <- Matrix::Matrix(
        matrix(
            c(
                alt_x[1, "cell_x1"],
                alt_x[1, "cell_x2"] + alt_y[1, "cell_y1"],
                alt_y[1, "cell_y2"],
                alt_x[2, "cell_x1"],
                alt_x[2, "cell_x2"] + alt_y[2, "cell_y1"],
                alt_y[2, "cell_y2"]
            ),
            nrow = 2,
            ncol = 3,
    ...
    # Verify merged matrices align with barcode ordering
    expect_equal(as.matrix(ref_count(merged)), as.matrix(expected_ref))
    # Verify merged alt counts align with barcode ordering
    expect_equal(as.matrix(alt_count(merged)), as.matrix(expected_alt))
})

test_that("merge_snpdata handles barcode merge with no overlapping barcodes", {
    data <- create_barcode_merge_test_data()
    x <- data$x
    y <- data$y
    ...
    # Verify alt counts match y when x has no matching barcodes
    expect_equal(unname(as.matrix(alt_count(merged))), unname(as.matrix(alt_count(y))))
})
...
test_that("merge_snpdata errors when no SNPs or cells are retained", {
    ...
    # Remove overlap in SNPs to trigger empty SNP set
    y_no_overlap <- SNPData(
        alt_count = alt_count(y),
        ref_count = ref_count(y),
        snp_info = data.frame(
            snp_id = c("snpC", "snpD"),
    ...
    expect_error(
        merge_snpdata(x, y_no_overlap, snp_join = "intersect", cell_join = "union"),
        "No SNPs to retain after merge"
    )

    # Remove overlap in cells to trigger empty cell set
    y_no_cells <- SNPData(
        alt_count = alt_count(y),
        ref_count = ref_count(y),
        snp_info = get_snp_info(y),
    ...
    # Verify error when intersect retains no cells
    expect_error(
        merge_snpdata(x, y_no_cells, snp_join = "union", cell_join = "intersect"),
...
test_that("get_donor_het_snpdata returns heterozygous SNPs for a donor", {
    ...
    # Verify only heterozygous SNPs for donor1 are retained
    expect_equal(rownames(result), c("snp1", "snp3"))
    # Verify only donor1 cells are retained
    expect_equal(ncol(result), 2)
})
...
# ==============================================================================
# Tests for filter_to_top_het_snp_per_gene
# ==============================================================================

test_that("filter_to_top_het_snp_per_gene filters to top het SNP per donor-gene", {
    # Setup - Create SNP data with multiple SNPs per gene with good het ratios
    # SNP1: donor1 (15:25 het), donor2 (5:45 hom)
    # SNP2: donor1 (10:30 het), donor2 (20:20 het)
    alt_count <- Matrix::Matrix(matrix(
        c(
            15, 5,   # SNP 1
            10, 20   # SNP 2
        ),
    ...
    ref_count <- Matrix::Matrix(matrix(
        c(
            25, 45,  # SNP 1
            30, 20   # SNP 2
        ),
    ...
    # Get donor counts
    donor_counts <- donor_count_df(snp_data, test_maf = FALSE)

    # Filter to top het SNP per gene
    result <- filter_to_top_het_snp_per_gene(donor_counts, snp_data, min_total_count = 10)

    # Verify result structure
    expect_s3_class(result, "tbl_df")
    # Verify we have one row per donor (both donors should have heterozygous SNPs at gene_A)
    expect_equal(nrow(result), 2)
    # Verify gene_name column is present
    expect_true("gene_name" %in% colnames(result))
    # Verify donor column is present
    expect_true("donor" %in% colnames(result))
    # Verify snp_id column is present
    expect_true("snp_id" %in% colnames(result))
})

test_that("filter_to_top_het_snp_per_gene selects SNP with highest coverage per donor-gene", {
    # Setup - Create SNP data where different SNPs have different coverage and are both het
    # SNP1: donor1 (15:25 het, total=40), donor2 (20:20 het, total=40)
    # SNP2: donor1 (5:45 hom, total=50), donor2 (5:45 hom, total=50)
    # So SNP1 should be selected for both donors (it's het, SNP2 is hom)
    # But let's make both het and varying by coverage:
    # SNP1: donor1 (30:20 het, total=50), donor2 (10:10 het, total=20)
    # SNP2: donor1 (10:30 het, total=40), donor2 (25:25 het, total=50)
    alt_count <- Matrix::Matrix(matrix(
        c(
            20, 10,  # SNP 1: donor1 total=50, donor2 total=20
            30, 25   # SNP 2: donor1 total=40, donor2 total=50
        ),
        nrow = 2, ncol = 2
    ), dimnames = list(c("snp_1", "snp_2"), c("cell_1", "cell_2")))
    ref_count <- Matrix::Matrix(matrix(
    ...
    # Verify SNP selection based on coverage
    # For donor_1: SNP1 has 50 total count, SNP2 has 40, so SNP1 should be selected
    donor1_result <- result %>% dplyr::filter(donor == "donor_1")
    # Verify SNP 1 was selected for donor_1 (highest coverage)
    expect_equal(donor1_result$snp_id, "snp_1")

    # For donor_2: SNP2 has 50 total count, SNP1 has 20, so SNP2 should be selected
    donor2_result <- result %>% dplyr::filter(donor == "donor_2")
    # Verify SNP 2 was selected for donor_2 (highest coverage)
    expect_equal(donor2_result$snp_id, "snp_2")
})

test_that("filter_to_top_het_snp_per_gene only includes heterozygous SNPs", {
    # Setup - Create SNP data with both het and hom SNPs
    # SNP1: donor1 (15:25 het), donor2 (5:45 hom)
    # SNP2: donor1 (2:48 hom), donor2 (5:45 hom)
    alt_count <- Matrix::Matrix(matrix(
        c(
            15, 5,   # SNP 1: het in donor1, hom in donor2
            2,  5    # SNP 2: hom in donor1, hom in donor2
        ),
        nrow = 2, ncol = 2
    ), dimnames = list(c("snp_1", "snp_2"), c("cell_1", "cell_2")))
    ref_count <- Matrix::Matrix(matrix(
    ...
    # Verify only donor_1 appears in result (donor_2 has no het SNPs at gene_A)
    expect_true("donor_1" %in% result$donor)
    expect_false("donor_2" %in% result$donor)
    # Verify only 1 row (1 het SNP for gene_A-donor_1)
    expect_equal(nrow(result), 1)
})

test_that("filter_to_top_het_snp_per_gene returns empty result when no het SNPs", {
    # Setup - Create SNP data with no heterozygous SNPs (all homozygous)
    # SNP1: donor1 (5:45 hom), donor2 (2:48 hom)
    alt_count <- Matrix::Matrix(matrix(
        c(5, 2),
        nrow = 1, ncol = 2
    ))
...
test_that("filter_to_top_het_snp_per_gene works with multiple genes", {
    # Setup - Create SNP data with multiple genes and donors
    # Gene A, SNP1: (15:25 het, total=40), (10:30 het, total=40)
    # Gene A, SNP2: (10:30 het, total=40), (10:30 het, total=40)
    # Gene B, SNP3: (15:25 het, total=40), (10:30 het, total=40)
    # Gene B, SNP4: (5:45 hom, total=50), (5:45 hom, total=50)
    alt_count <- Matrix::Matrix(matrix(
        c(
            15, 10,  # SNP 1: gene_A, donor1/donor2
            10, 10,  # SNP 2: gene_A
            15, 10,  # SNP 3: gene_B
            5,  5    # SNP 4: gene_B
        ),
        nrow = 4, ncol = 2
    ), dimnames = list(c("snp_1", "snp_2", "snp_3", "snp_4"), c("cell_1", "cell_2")))
    ref_count <- Matrix::Matrix(matrix(
        c(
            25, 30,  # SNP 1: gene_A
            30, 30,  # SNP 2: gene_A
    ...
    snp_info <- data.frame(
        snp_id = c("snp_1", "snp_2", "snp_3", "snp_4"),
        gene_name = c("gene_A", "gene_A", "gene_B", "gene_B"),
        pos = c(100, 200, 300, 400),
        stringsAsFactors = FALSE
    )
    ...
    # Verify we have 3 rows (gene_A-donor1, gene_A-donor2, gene_B-donor1 are het, gene_B-donor2 has no het SNPs)
    expect_equal(nrow(result), 3)
    # Verify both genes are present
    expect_true(all(c("gene_A", "gene_B") %in% result$gene_name))
    # Verify donor_1 is present but donor_2 is missing gene_B
    expect_true("donor_1" %in% result$donor)
    # SNP3 is the only het SNP for gene_B donor_1
    gene_b_results <- result %>% dplyr::filter(gene_name == "gene_B")
    expect_equal(gene_b_results$snp_id, "snp_3")
})
